...
class GAN(object):
    def __init__(self, params):
        ...
        with tf.variable_scope('G'):
            self.z = tf.placeholder(tf.float32, shape=(params.batch_size, 9))
...
def train(model, data, gen, params):
    ...
    with tf.Session() as session:
        ...
        model_vars = [v for v in model_vars if v.name.startswith('D/d0/w:0')]
        learned_rep_tensor = model_vars[0]
        ...
        for step in range(params.num_steps + 1):
            ...
            ginput = np.reshape(z, (params.batch_size, 1))
            learned_rep = session.run(learned_rep_tensor)
            learned_rep = learned_rep*np.ones([ params.batch_size, learned_rep.shape[1]])
            ginput = np.hstack((ginput, learned_rep))
        ...
        if params.anim_path:
            ...
            samps = samples(model, session, data, learned_rep, gen.range, params.batch_size)
