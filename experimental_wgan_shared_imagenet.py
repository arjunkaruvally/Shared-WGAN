...
class DataDistribution(object):
    ...
    def sample(self, N, out_y=False):
        ...
        # print np.max(sample_inputs)

        sample_inputs = (sample_inputs + 1)/2

        # plt.imshow(sample_inputs[0])
        # plt.show()
...
def main(args):
    ...
    celebFaces = DataDistribution()

    # celebFaces.sample(64)

    model = GAN(args)

    if args.mode=="train":
        train(model, celebFaces, GeneratorDistribution(range=8), args)
