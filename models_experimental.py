...
class GeneratorDistribution(object):
    ...
    def sample(self, N):
        return (np.random.uniform(-1, 1, size=[N, self.dim]))
...
def generator(input, h_dim, params):
    ...
    h0 = lrelu(g0_bn(conv2d(h0t, h_dim, d_h=1, d_w=1, name='gshared')))
...
class GAN(object):
    def __init__(self, params):
        ...
        with tf.variable_scope('G'):
            self.z = tf.placeholder(tf.float32, shape=(params.batch_size, params.input_dim))
...
def test(model, gen, params):
    with tf.Session() as session:
        ...
        for i in range(params.test_count):
            ...
            plot_gen_out(session, model, gen, params.batch_size, "test/test_"+str(i), params=params)
...
def main(args):
    ...
    if args.mode=="train":
        train(model, data, GeneratorDistribution(args.input_dim), args)

    elif args.mode == "test":
        test(model, GeneratorDistribution(args.input_dim), params=args)
